...
def main(attacks, argument_to_check):
    ...
    while valid_argument == False:
        try:
            if any(x not in arguments for x in set(argument_to_check)):
                print(f"Please enter a valid argument, containing any of {arguments}")
            elif len(set(argument_to_check)) > 2:
                print("Please enter at most two arguments.")
            else:
                valid_argument = True
                break
        except ValueError:
    ...
    if is_in_admissible_set:
        print(f"The argument {set(argument_to_check)} is credulously accepted under admissible semantics.")
    else:
        print(f"The argument {set(argument_to_check)} is not credulously accepted under admissible semantics.")
